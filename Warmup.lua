...
function Warmup:PutOut(txt, color, time, mem)
	local outstr = ((not time) and string.format("%s%s (%d KB)", color, txt, mem))
		or (not mem and string.format("%.3f 秒 | %s%s", time, color, txt))
		or string.format("%.3f 秒 | %s%s (%d KB)", time, color, txt, mem)
...
function Warmup:PutOutAO(ao)
	...
	local outstr = string.format("%s%.3f 秒|r | %s (%s%d KB|r)%s", tc, ao.time, ao.name, mc, ao.mem, gc)
	...
	for i=1,GetNumAddOns() do
		if IsAddOnLoaded(i) then
			if GetAddOnInfo(i) ~= self.myname then
				self.frame:AddMessage("Warmup插件加载之前: ".. GetAddOnInfo(i))
				self.myframe:AddMessage("Warmup插件加载之前: ".. GetAddOnInfo(i))
			end
		end
	end
...
function Warmup:OnUpdate()
	if self.gcalerts then
		...
		if (Warmup.gc and gc ~= Warmup.gc) then
			local outstr = string.format("GC fired | mem %d --> %d | 阀值 %d --> %d", Warmup.mem, mem, Warmup.gc, gc)
...
function Warmup:ADDON_LOADED()
	...
	-- 初始化
	self.longestaddon = self.longestaddon or arg1
	self.biggestaddon = self.biggestaddon or arg1
...
function Warmup:VARIABLES_LOADED()
	...
	self:PutOut("插件负载", self.green, self.varsloadtime - self.inittime, self.lastmem - self.initmem)

	-- 检查nil值
	if self.longestaddon then
		self:PutOut("加载时间最长的插件: ".. self.longestaddon, self.yellow, self.longesttime)
	end
	if self.biggestaddon then
		self:PutOut("占内存最大的插件: ".. self.biggestaddon, self.yellow, nil, self.biggestmem)
	end
...
function Warmup:PLAYER_ENTERING_WORLD()
	if self.inittime and self.logging then
		...
		self:PutOut("世界接口", self.green, entrytime - self.varsloadtime)

		if self.sv.time then self:PutOut("总时间", self.green, entrytime - self.sv.time)
		else self:PutOut("总时间", self.green, entrytime - Warmup.inittime) end
